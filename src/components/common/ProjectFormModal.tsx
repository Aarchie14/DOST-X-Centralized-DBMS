...
/**
 * Interface for ProjectFormModal component props.
 */
 ...
 * Provides a standardized modal interface for creating or updating project entries.
 */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl w-full max-w-md p-6 relative z-10 shadow-xl animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                {Object.values(DEPARTMENTS)
                  .filter((d) => d !== "All department")
                  .map((d) => (
                    <option key={d} value={d} className="text-slate-700">
                      {d}
                    </option>
                  ))}
              </select>
          ...
          {/* Sector Category Field */}
          <div>
            <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wide mb-1.5">
              Sector Area / Category
            </label>
            <select
              required
              ...
              onChange={(e) =>
                setNewProject({
                  ...newProject,
                  sectorCategory: e.target.value as any,
                })
              }
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 outline-none focus:bg-white focus:border-[#00aeef] cursor-pointer invalid:text-slate-400"
            >
          ...
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 mt-5">
            ...
            <button
              type="submit"
              className="px-5 py-2 text-xs font-bold text-white bg-[#00aeef] hover:bg-sky-600 rounded-xl transition-all shadow-2xs active:scale-95 cursor-pointer"
            >
